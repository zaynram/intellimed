...
class TrustGenerator:
    ...
    @property
    def trust_name(self):
        return self.g("trust_id.desired_trust_name",
    ...
    def trustee_names(self):
        ...
        if t == "both": return f"{self.husband} and {self.wife}"
        if t == "husband": return self.husband
        if t == "wife": return self.wife
        return t
    ...
    def withdrawal_text(self):
        ...
        if not steps:
            return ("At intervals to be determined, the beneficiary may withdraw "
        ...
        for i, s in enumerate(steps):
            ...
            if i == 0:
                parts.append(f"{pct}% of accumulated trust income and principal, {timing}")
            else:
                parts.append(f"increased by {pct}% of accumulated trust income and "
        ...
        for i, p in enumerate(parts):
            text += f"    ({chr(97+i)}) {p};\n\n"
        text += ("These rights are cumulative. A beneficiary exercises this right by written "
    ...
    def _title_page(self):
        self.blank(4)
        for txt, sz, b in [("THE", 16, True), (self.trust_name.upper(), 18, True),
                            (f"Dated: {self.trust_date}", 12, False)]:
            p = self.doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(txt)
            r.font.name = 'Arial'; r.font.size = Pt(sz); r.bold = b
        self.blank(6)
        for txt in ["Prepared by:", "Crosby and Crosby LLP",
                     "3815 N Mulford Rd. 4", "Rockford, IL 61114", "(815) 367-6432"]:
            p = self.doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(txt)
            r.font.name = 'Arial'; r.font.size = Pt(11)
            if "Crosby" in txt and "Prepared" not in txt: r.bold = True
    ...
    def _article_1(self):
        ...
        if children:
            ...
            self.body("All references to \u201cour children\u201d mean these children. "
        ...
        self.body(f"This Trust shall be known as the {self.trust_name}. For transfers, "
        ...
        self.body("Our Trustee may use a certification of trust instead of this instrument. "
        ...
        self.body("By executing this instrument, we transfer the property described in the "
                  "attached schedules. At the time of drafting, those assets include:")
        for i, a in enumerate(self.asset_list(), 1):
            self.indent(f"{i}. {a}")
        self.body("This instrument also considers additional assets we may obtain before "
                  "this instrument becomes irrevocable.")
        self.body("Our Trustee accepts and agrees to hold transferred property as Trust "
        ...
        self.body("We create this revocable living trust for our benefit while living, and "
        ...
        self.body(f"We intend to qualify as a Grantor Trust under IRC Sections 671-677. "
                  f"The Tax ID will be {self.ssn_owner_name}\u2019s Social Security number.")

        self.h2("1.8 Powers Reserved by Us as Grantors")
        self.body("Either of us may act for the Trust without the other\u2019s consent. "
    ...
    def _article_2(self):
        ...
        self.body("A Trustee may resign by written notice to either of us, or if both "
                  "incapacitated or deceased, to beneficiaries and co-trustees.")

        self.h2("2.2 General Succession")
        self.body("If either of us ceases to act as Trustee, the surviving spouse continues "
                  "as sole Trustee. If both are unable, succession is:")
        for s in self.data.get("successor_trustees", []):
            self.indent(f"{s['order']}: {s['name']}"
        ...
        self.body("If no designated Trustee can serve: we may jointly appoint one; if only "
        ...
        self.h2("2.4 Removal")
        self.body("We may jointly remove any Trustee. During one\u2019s incapacity, the other "
        ...
        self.body("A written declaration of incapacity by a co-Trustee or designated successor, "
        ...
        self.body("Successor Trustees have all rights and obligations of the initial Trustee. "
    ...
    def _article_3(self):
        ...
        self.body("While both alive, we may revoke or amend in whole or part by written "
                  "agreement signed by both of us.")

        self.h2("3.2 Surviving Spouse\u2019s Amendment Rights")
        ap = self.g("surviving_amendment", "full")
        if ap == "full":
            self.body("The surviving spouse has full power to amend or revoke the entire Trust.")
        elif ap == "limited":
            self.body("The surviving spouse may amend only the Survivor\u2019s Trust and "
        ...
        self.body("While both alive, we may use or distribute Trust property as if owned "
                  "outside the Trust.")

        self.h2("3.4 Administration During Incapacity")
        self.body("If one of us is incapacitated, our Trustee shall distribute for: "
        ...
        self.body("Incapacity is established by two licensed physicians, court declaration, "
    ...
    def _article_4(self):
        ...
        self.body("After the first death, the survivor\u2019s interest is the "
        ...
        self.body("The Trust becomes irrevocable as to the deceased\u2019s property. "
                  "A separate Tax ID may be needed.")

        self.h2("4.3 Payment of Expenses and Taxes")
        self.body("Our Trustee shall pay: (a) funeral and memorial expenses; (b) enforceable "
        ...
        self.body(f"Our Trustee shall distribute tangible personal property to {recip} in "
                  f"substantially equal shares. If unable to agree within six months, the "
                  f"Trustee decides.")
        for b in self.data.get("specific_bequests", []):
            self.indent(f"We direct that {b['item']} be distributed to {b['recipient']}"
                       + (f". {b['instructions']}" if b.get('instructions') else "."))

        self.h2("4.5 Tax Elections and Portability")
        self.body("Our Trustee may make necessary tax elections. If the deceased\u2019s "
        ...
        if self.g("probate_coordination", str(True)):
            ...
            self.body("Our Trustee may rely on the Personal Representative\u2019s requests, "
    ...
    def _article_5(self):
        ...
        self.body("Our Trustee allocates the deceased\u2019s remaining property to the "
                  "Survivor\u2019s Trust.")

        self.h2("5.1 Trustee")
        self.body("The surviving Grantor may serve as sole Trustee and may remove or "
                  "replace the Trustee at any time.")

        self.h2("5.2 Right to Amend")
        self.body("Subject to Section 3.2, the surviving Grantor may amend, restate, or "
                  "revoke the Survivor\u2019s Trust in writing.")

        self.h2("5.3 Income and Principal")
        self.body("All net income distributed quarter-annually to the surviving Grantor. "
        ...
        if poa == "general":
            self.body("The surviving Grantor may appoint all or any portion at death among "
        ...
        self.body("The Survivor\u2019s Trust becomes irrevocable. Administered per Article 4, "
    ...
    def _article_6(self):
        ...
        stxt = "health, education, maintenance, or support" if std == "hems" else "any purpose"
        self.body(f"Our Trustee shall distribute income and principal as determined necessary "
                  f"for each beneficiary\u2019s {stxt}. Undistributed income added to principal.")
        self.body("We request our Trustee be liberal in making distributions, considering "
        ...
        if dp == "per_stirpes_beneficiary":
            self.body("The deceased beneficiary\u2019s share passes per stirpes to their "
                      "descendants, then to our descendants, then per Article 7.")
        elif dp == "per_stirpes_grantors":
            self.body("The share passes per stirpes to our descendants, then per Article 7.")
        elif dp == "redistribute":
            self.body("The share is redistributed equally among surviving named beneficiaries.")
    ...
    def _article_7(self):
        ...
        if rc == "intestacy":
            self.body(f"If no one is qualified to receive distribution, property passes "
                      f"per {self.state} intestacy laws.")
        elif rc == "charity":
            self.body("If no one is qualified, property passes to "
                      f"{self.g('remote_contingent_charity','[NAMED CHARITY]')}.")

    # --- Article 8 ---
    def _article_8(self):
        self.h1("Article 8: Underage and Incapacitated Beneficiaries")
        self.body("For beneficiaries under 18 or incapacitated, our Trustee may distribute "
    ...
    def _article_10(self):
        ...
        self.body("Our Trustee may make cash, in-kind, or mixed distributions in any "
                  "proportion without regard to tax attributes or beneficiary consent.")

        self.h2("10.2 No Court Proceedings; No Bond")
        self.body("Our Trustee shall administer with efficiency and freedom from judicial "
                  "intervention. No bond is required.")
        if self.g("dispute_resolution") == "mediation_arbitration":
            self.body("Disputes should be resolved by mediation, then arbitration per the "
                      "Uniform Arbitration Act.")

        self.h2("10.3 Trustee Compensation")
        comp = self.g("trustee_compensation", "reasonable")
        if comp == "reasonable":
            self.body("Individual Trustees receive fair and reasonable compensation. "
                      "Corporate fiduciaries per their fee schedule. All may be reimbursed.")
        elif comp == "none":
            self.body("Family Trustees serve without compensation. Corporate fiduciaries "
                      "per their fee schedule. All may be reimbursed.")

        self.h2("10.4 Accounting")
        self.body("After the first death, annual accounting to Income Beneficiaries unless "
                  "waived. Objections within 60 days. Records available at reasonable times.")

        self.h2("10.5 Liability Limitations")
        self.body("No Trustee liability unless bad faith shown by clear and convincing evidence.")

        self.h2("10.6 Merge, Sever, or Terminate Trusts")
        self.body("Our Trustee may merge, sever, or terminate trusts when economical. "
    ...
    def _article_11(self):
        ...
        self.body(f"Our Trustee has all powers granted by this Trust and {self.state} law, "
        ...
        self.body("An Interested Trustee is limited to HEMS distributions per IRC Sections "
    ...
    def _article_12(self):
        ...
        self.body("Each trust terminates 21 years after the death of the last descendant of "
        ...
        if self.g("spendthrift", str(True)):
            ...
            self.body("All trusts are spendthrift trusts. No beneficiary may assign, "
        ...
        if self.g("no_contest", str(True)):
            ...
            self.body("Any person who contests this Trust forfeits their share and is "
                      "deemed to have predeceased the last of us.")

        self.h2("12.4 Survivorship Presumption")
        self.body("If we die simultaneously, each is deemed to have predeceased the other. "
                  "A beneficiary dying within 30 days of a Grantor is deemed to have "
                  "predeceased the Grantor.")

        self.h2("12.5 Definitions")
        defs = [
            ("Adopted Persons", "Legally adopted before 18 have same rights as natural children."),
            ("Descendants", "Children, grandchildren, etc. Not nieces or nephews."),
            ("Education", "An ascertainable standard: schooling, college, vocational, tuition, room, board, fees."),
            ...
            ("Independent Trustee", "Not related or subordinate per IRC Section 672(c)."),
            ("Per Stirpes", "Divided among children and deceased children\u2019s descendants equally."),
        ]
        for term, defn in defs:
            p = self.doc.add_paragraph()
            p.paragraph_format.space_after = Pt(4)
            rb = p.add_run(f"{term}: "); rb.bold = True; rb.font.name = 'Arial'; rb.font.size = Pt(11)
            rn = p.add_run(defn); rn.font.name = 'Arial'; rn.font.size = Pt(11)

        self.h2("12.6 Rules of Construction")
        self.body(f"Executed in counterparts. Governed by {self.state} law. Headings for "
    ...
    def _signatures(self):
        ...
        self.blank()
        self.body(f"Acknowledged before me on {self.trust_date}, by {self.husband}, "
    ...
    def _schedules(self):
        ...
        if bq:
            for b in bq:
                self.indent(f"{b['item']} \u2192 {b['recipient']}"
                           + (f" ({b['instructions']})" if b.get('instructions') else ""))
        else:
