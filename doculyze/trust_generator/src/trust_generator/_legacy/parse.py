...
class QuestionnaireParser:
    ...
    def _cell_text(self, cell):
        ...
        hints = [
            "e.g.,", "MM/DD/YYYY", "XXX-XX-XXXX", "Street, City, State, ZIP",
            "If different from Husband", "Husband / Wife", "Default: Illinois",
            "Yes / No", "Describe briefly", "Joint / H / W", "$",
            "e.g., Checking, IRA, 401k", "Trust / POD to ___",
            "e.g., Winnebago", "e.g., United States", "e.g., John Andrew Doe",
            "e.g., Jane Susan Doe", "e.g., The Doe Family Trust",
            "H / W / Both", "H / W", "Pension / Annuity",
            "e.g., upon college graduation", "e.g., 1 year after funding",
            "e.g., 2 years after funding", "e.g., 5 years after funding",
            "e.g., 50%", "e.g., additional 25%", "e.g., remaining 25%", "%",
            "H / W / Joint", "Yes / No",
        ]
        for hint in hints:
            if text == hint:
    ...
    def _parse_tables(self):
        for table in self.doc.tables:
            ...
            if "field" in headers and "answer" in headers:
                ...
                self._parse_list_table(table, "real_property",
                    ["address", "value", "equity", "transfer"])
            elif "institution" in headers:
                self._parse_list_table(table, "financial_accounts",
                    ["institution", "type", "value", "owner", "designation"])
            elif "year" in hstr and "make" in hstr:
                self._parse_list_table(table, "vehicles",
                    ["description", "vin", "value", "owner", "transfer"])
            elif "company" in headers and "policy" in hstr:
                self._parse_list_table(table, "insurance_policies",
                    ["company", "policy_number", "benefit", "insured", "beneficiary"])
            elif "source" in hstr and "type" in headers:
                self._parse_list_table(table, "pensions",
                    ["source", "type", "value", "owner", "survivor"])
            elif "item description" in hstr:
                self._parse_list_table(table, "valuables",
                ...
                self._parse_list_table(table, "specific_bequests",
                    ["item", "recipient", "instructions"])
                ...
                self._parse_list_table(table, "other_beneficiaries",
        ...
        for row in table.rows[1:]:
            ...
            if cells[0].strip() and cells[0].strip().upper() not in ("TOTAL:", "TOTAL"):
                shares.append({
                    ...
                    "share": cells[2].strip().replace('%', '').strip() if len(cells) > 2 else "",
    ...
    def _parse_checkboxes(self):
        ...
        mapping = {
            ...
            "payable directly to surviving spouse": ("insurance_strategy", "spouse_then_children"),
        ...
        for para in self.doc.paragraphs:
            ...
            if not text:
                continue
            is_checked = text.startswith(checked) or text.startswith("[X]") or text.startswith("[x]")
            if is_checked:
    ...
    def _parse_text_blocks(self):
        ...
        end_markers = ["section ", "article ", "for office use", "signatures",
                       "maps to:", "would you like", "are there", "definition of",
                       "method if beneficiaries", "by signing below"]
