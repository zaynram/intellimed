...
ELECTION_TOOLTIPS: dict[str, str] = {
    ...
    "elections.spendthrift": (
        "Prevents creditors from accessing trust assets. "
        "Recommended for most trusts."
    ),
    ...
    def populate(self, data: dict[str, str | bool]) -> None:
        ...
        for field in self._fields:
            ...
            if isinstance(field, CheckboxField):
                field.set_value(value if isinstance(value, bool) else str(value).lower() == "true")
            else:
...
def trust_data_to_flat(td: TrustData) -> dict[str, str | bool]:
    ...
    for parent_name in ("party_a", "party_b", "grantor", "trust_id", "marriage", "office"):
        ...
        for field_name, field_value in parent_obj.model_dump(mode="json").items():
            flat[f"{parent_name}.{field_name}"] = str(field_value) if field_value is not None else ""
    ...
    for field_name, field_value in td.text_blocks.model_dump().items():
        flat[f"text_blocks.{field_name}"] = str(field_value) if field_value is not None else ""
...
def flat_to_trust_data(flat: dict[str, str | bool]) -> TrustData:
    ...
    kwargs["trust_type"] = (
        TrustType(trust_type_val) if isinstance(trust_type_val, str) else TrustType.JOINT
    )
    ...
    for key, val in flat.items():
        ...
        if field_name in _BOOL_ELECTIONS:
            election_data[field_name] = val if isinstance(val, bool) else str(val).lower() == "true"
        else:
