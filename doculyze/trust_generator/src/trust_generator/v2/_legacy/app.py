...
try:
    from trust_generator.v2._legacy import QuestionnaireParser, TrustGenerator
except ImportError:
...
def run_gui():
    ...
    class App:
        def __init__(self, root):
            ...
            ttk.Label(main, text="Family Trust Generator", style="Title.TLabel").pack(
                pady=(0, 5)
            )
            ttk.Label(main, text="Crosby and Crosby LLP", style="Sub.TLabel").pack(
                pady=(0, 20)
            )
            ...
            # Input
            f1 = ttk.LabelFrame(
                main, text="Step 1: Select Completed Questionnaire", padding=10
            )
            ...
            ttk.Button(f1, text="Browse...", command=self._browse_in).pack(
                side=tk.RIGHT
            )
            ...
            ttk.Button(f2, text="Browse...", command=self._browse_out).pack(
                side=tk.RIGHT
            )
            ...
            ttk.Label(
                main, textvariable=self.status, font=("Arial", 10, "italic")
            ).pack(pady=5)
        ...
        def _browse_in(self):
            ...
            if p:
                ...
                self.output_var.set(
                    os.path.join(
                        d, f"Family_Trust_{datetime.now().strftime('%Y%m%d')}.docx"
                    )
                )
        ...
        def _generate(self):
            ...
            if not inp:
                messagebox.showerror("Error", "Select a questionnaire.")
                return
            if not out:
                messagebox.showerror("Error", "Choose output location.")
                return
...
def run_cli(args):
    ...
    if not os.path.exists(args.questionnaire):
        print(f"ERROR: {args.questionnaire} not found")
        sys.exit(1)
    ...
    print(
        f"  Assets: {sum(len(data.get(k, [])) for k in ['real_property', 'financial_accounts', 'vehicles', 'valuables', 'insurance_policies', 'pensions'])}"
    )

    if args.dump_data:
        print("\n--- Parsed Data ---")
        safe = {
            k: (v if isinstance(v, (list, dict, bool)) else str(v))
...
def main(mode: Literal["auto", "cli"] = "auto"):
    ...
    parser.add_argument(
        "--dump-data", action="store_true", help="Print parsed data as JSON"
    )
