...
from trust_generator.config import AppConfig, FirmConfig
from trust_generator.generators import generate_printable_questionnaire
...
def test_no_placeholder_text(tmp_path: Path) -> None:
    ...
    for hint in forbidden:
        assert hint.lower() not in text.lower(), (
            f"Placeholder text found: {hint!r}"
        )
